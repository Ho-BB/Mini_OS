...
void h743_reset_isr(){
  ...
  echo_task.pid = 1;
  echo_task.entry = (u32) echo;
  echo_task.sp = ((u32) __AXI_SRAM_END) - 4096;

  task_init_stack(&idle_task);
  task_init_stack(&echo_task);
  ...
  elected = sched_add(&kernel_data.sched, &echo_task, &changed);
  ...
  kernel_data.tim6_7_conf.us = 10000;
...
void echo(){
  ...
  while(1){
    syscall_usart_read(buf, 16, &read);
