...
/*
  ...
  __AXI_SRAM_START[2] : u32 (usart rx num_data byte number)
  __AXI_SRAM_START[3] : u32 (usart rx num_data)
  ...
  __AXI_SRAM_START[14] : transfer_queue_t * (tasks waiting for rx)
  __AXI_SRAM_START[15] : transfer_queue_t * (rx waiting to be read)
  __AXI_SRAM_START[16] : u8 * (rx in progress)
  __AXI_SRAM_START[17] : u32 (rx in progress len)
*/
...
#define INUMDATA         (__AXI_SRAM_START[2])
#define NUMDATA          (__AXI_SRAM_START[3])
#define TASK_WAIT_RX     ((transfer_queue_t *) __AXI_SRAM_START[14])
#define RX_WAIT_TASK     ((transfer_queue_t *) __AXI_SRAM_START[15])
#define RX_IN_PROGRESS   ((u8 *) __AXI_SRAM_START[16])
#define RX_IN_PROGRESS_LEN (__AXI_SRAM_START[17])
...
void h743_reset_isr(){
  ...
  transfer_queue_t tq;
  freelist_alloc_t fa;
  transfer_queue_t task_wait_rx, rx_wait_task;
  ...
  tq_init(&task_wait_rx, ((transfer_t **) fa_alloc(&fa, 32)), 8);
  tq_init(&rx_wait_task, ((transfer_t **) fa_alloc(&fa, 32)), 8);
  
  __AXI_SRAM_START[0] = (u32) &sched;
  __AXI_SRAM_START[1] = (u32) &fa;
  __AXI_SRAM_START[2] = 0; // byte number
  __AXI_SRAM_START[3] = 0; // num_data
  __AXI_SRAM_START[4] = (u32) &tq;
  __AXI_SRAM_START[12] = (u32) &dma_conf;
  __AXI_SRAM_START[13] = (u32) &dmamux_conf;
  __AXI_SRAM_START[14] = (u32) &task_wait_rx;
  __AXI_SRAM_START[15] = (u32) &rx_wait_task;
  ...
  u32 c = h743_usart_receive(USART2);
  h743_usart_wait_rxne(USART2);
  c = h743_usart_receive(USART2);
  h743_usart_wait_rxne(USART2);
  c = h743_usart_receive(USART2); //because of usart_com_util_2
...
void setup_dma_transfer_rx(u8 *buf, u32 len){
  ...
  DMA_CONF->mem_address_0 = (u32) buf;
  DMA_CONF->fifo_mode = FALSE;
  h743_dma_conf(DMA1, 1, DMA_CONF);

  DMAMUX_CONF->request = DMAMUX_USART2_RX;
  h743_dmamux_conf(DMAMUX1, 1, DMAMUX_CONF);
...
u32 dma_handler_rx(u32 old_sp){
  ...
  if(pending_irq & DMA_IRQ_TC){
    ...
    h743_usart_enable_irq(USART2, USART_IRQ_RXNE);
    h743_usart_disable_dma_rx(USART2);
  }
  else if(pending_irq & DMA_IRQ_TE){
    h743_dma_eoi(DMA1, 0, DMA_IRQ_TE);
  }
