...
void shell(u32 argc, u8 **argv){
  ...
  while(1){
    syscall_usart_write((u8 *) "[os-h743]$ ", 11);
    syscall_usart_read(buf, 128, &read);
    buf[read - 1] = '\0'; //replace '\n' with '\0'
    syscall_launch(buf);
  }
...
void hello(u32 argc, u8 **argv){
  ...
  while(1){
    syscall_usart_write((u8 *) "hello : ", 8);
    for(i = 1; i < argc; i++)
      syscall_usart_write(argv[i], str_len(argv[i]));
    syscall_sleep(2000);
  }
}
