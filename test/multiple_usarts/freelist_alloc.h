...
/*
  ...
  updated to cover the merged block(s). 
  In the case where the block to be freed can be merged with the previous block,
  the len field of the previous block is updated to cover the length of the
  freed block + 8 (metadata size).
  In the case where the block to be freed can be merged with the following 
  block, the len field of the freed block is updated to cover the length of the
  following block + 8 and the next field of the previous block is updated to 
  point to the newly freed block, the next field of the new block takes 
  the value of the next field of the merged block.
  In the case where the block to be freed can be merged with both the previous
  and the following block, the len field of the previous block is updated to 
  cover the length of both blocks + 16 and the next field of the previous block
  takes the value of the next field of the following block.
*/
